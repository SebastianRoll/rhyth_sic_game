...
class Encoder(object):
    def __init__(self, pin_clk, pin_dt, pin_mode=None, clicks=1,
        ...
        self.pin_clk = Pin(pin_clk, Pin.IN, Pin.PULL_UP, trigger=mode, handler=self._callback)
        self.pin_dt = Pin(pin_dt, Pin.IN, Pin.PULL_UP, trigger=mode, handler=self._callback)
        ...
        self._value_last_checked = self.value
    ...
    def turned_left(self):
        turned = self.value < self._value_last_checked
        if turned:
            self._value_last_checked = self.value
        return turned
