from utils import timed_function, free
...
class RhythGame:
    def __init__(self, pin_ws2812, pin_ws2813, pin_outer, touch_driver=None, brightness=255, debug=False, song_list=None):
        ...
        self.ws2812_np = NeoPixel(Pin(pin_ws2812), 34*4 + 12)
    ...
    def play_song(self, title='dr_chaos', delay_ms=500, difficulty='easy'):
        self.ring_led[:] = array('L', [0x0000aa]*len(self.ring_led))
            ...
            try:
                ...
                if self.o % 10 == 0:
                    ...
                    if last_active_ts > 0 and game.time_passed() > game.beat_last_ts:
                        ...
                        break
                if self.o % 301 == 0:
                    print(self.o, free(full=True))
                    # gc.collect()
                #     print(self.o, "GARBAGE COLLECT")
                self.tick_animation()
                self.refresh_leds()
    ...
    def refresh_leds(self):
        ...
        for note_id, mv in self.notes_led.items():
            ...
            if len(positions) > 20:
                print("POS", note_id, len(positions))
